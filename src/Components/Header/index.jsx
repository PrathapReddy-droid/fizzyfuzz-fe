...
const Header = () => {
  ...
  const logout = () => {
    ...
    fetchDataFromApi(`/api/user/logout?token=${localStorage.getItem('refreshToken')}`, { withCredentials: true }).then((res) => {
      if (res?.error === false) {
    ...
    <>
      ...
      >
        <div className="part1 flex items-center gap-4">
          ...
          >
            <RiMenu2Line className="text-[18px] text-[rgba(255,255,255,0.8)]" />
          </Button>
        </div>
        ...
        <div className="part2  flex items-center justify-end gap-5">
          ...
          {context.isLogin === true ? (
            <div className="relative">
              <div
                className="rounded-full border-2 border-purple-300 w-[35px] h-[35px] overflow-hidden cursor-pointer"
